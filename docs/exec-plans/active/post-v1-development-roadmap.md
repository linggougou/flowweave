...
- 生命周期：P2.8 post-v1 产品化已完成；vNext-0 设计门禁已获用户明确授权并开启。
...
- vNext 只解冻设计，不开放业务实现；P3 深度页面/网络理解与 P4 AI 编排继续冻结。
...
2. Studio 单条执行记录删除及受控 runs 清理（P2.7 已完成；Local API 缺少破坏性授权，因此不向 Web 暴露删除）。
3. Studio / Web Flow 版本 JSON diff（P2.7 已完成；共享安全只读展示，不引入复杂编辑器）。
...
1. project-knowledge 通过真实项目、execution 和 stepIndex 推导受控 `step-N.png`，验证 symlink、普通文件、大小、PNG signature 与读取身份。
2. Electron preload / IPC 只接受业务 ID，不接收或返回绝对路径；移除通用 `openPath`。
...
该阶段会改变 Flow 产品模型与 runtime 执行协议。用户已确认进入设计阶段，当前实施真源为 `docs/exec-plans/active/vnext-0-design-gate.md`。设计 Judge PASS 前不创建输入节点实现，不解冻 P3/P4。
...
8. P2.8 受控截图预览已完成并归档；当前进入 vNext-0 协议设计门禁，不进入编码。
