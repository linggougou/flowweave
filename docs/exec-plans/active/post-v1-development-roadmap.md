...
- P2.5、P2.6 已完成并归档；P2.7 本地实施与验收已通过，等待远端双矩阵会签后归档。
...
2. Studio 单条执行记录删除及受控 runs 清理（P2.7 本地门禁已通过；Local API 缺少破坏性授权，因此不向 Web 暴露删除）。
3. Studio / Web Flow 版本 JSON diff（P2.7 本地门禁已通过；共享安全只读展示，不引入复杂编辑器）。
