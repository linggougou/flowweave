...
- [x] 当前与历史扩展裸 JSON 均可校验导入。
- [x] 导入始终为新副本，无静默覆盖和半写入。
- [x] 已知敏感默认值、本机路径与明显 URL 凭据按合同处理并返回 warnings。
- [x] Studio 文件交互不暴露任意路径能力，取消/非法/超限均无副作用。
- [x] 扩展与 Studio 共享导出合同。
- [x] Web 重命名持久化且无异步串选。
- [x] 导出 → 空项目导入 → 运行闭环通过。
- [x] recorded replay 保持 `25/25`，本地 Node 20/24 与安全门禁通过。
- [x] 集成分支与 main 的 Node 20/24 CI 双绿会签。
- [x] P3/P4、vNext 和破坏性删除能力未被混入。
- [x] 所有 Agent、worktree 与临时进程已回收。

当前状态：全部 DoD 与 S7 会签已通过；计划于 2026-08-23 归档。P2.7、P3/P4 与 vNext 未自动开启。

## 8. 最终会签

- 最终独立总审：PASS，无 P0/P1。
- 集成分支最终 CI：run `32642875346`，Node 20 job `97202559089`、Node 24 job `97202559229` 均成功。
- main CI：run `32643072629`，Node 20 job `97203054439`、Node 24 job `97203054597` 均成功。
- 归档判断：P2.6 代码、测试、构建、安全审计、真实往返、本地双版本与远端双矩阵全部通过。
